...
[![crash suite](https://img.shields.io/badge/crash_suite-22%2F22_failure_modes_caught-2ea44f)](docs/CRASHTEST.md) <!-- cases:badge -->
...
Claims about supervisors are cheap. `bun run crashtest` runs <!-- cases:count -->22<!-- /cases:count --> deliberately broken cases under nightshift, each with the limit that should catch it, and passes only if the report says the right thing **and no process from the case is left alive**.
