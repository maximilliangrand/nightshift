...
Claims about supervisors are cheap. `bun run crashtest` runs <!-- cases:count -->26<!-- /cases:count --> deliberately broken cases under nightshift, each with the limit that should catch it, and passes only if the report says the right thing **and no process from the case is left alive**.
