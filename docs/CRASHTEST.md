...
- Anything that needs a real model. `budget-blower.ts`, `codex-blower.ts` and the two `openclaw-*-blower.ts` agents fake the streams; the metering itself is covered by `test/claude-meter.test.ts` against events captured from a real run, by `test/codex-meter.test.ts` against the shapes in the Codex sources and docs, and by `test/openclaw-meter.test.ts` against an envelope captured from OpenClaw 2026.4.10 and a session store and transcript in the shape the gateway writes (see [ADAPTERS.md](ADAPTERS.md) and [ADAPTERS-openclaw.md](ADAPTERS-openclaw.md)).
